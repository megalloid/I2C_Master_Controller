...
| Linux + консоль на OLED (после Vivado §18) | [GUIDE_LINUX_ZYNQ_MINI_OLED.md](doc/GUIDE_LINUX_ZYNQ_MINI_OLED.md) | FSBL, Buildroot с нуля (DTS/драйвер вручную), BOOT.BIN, fbcon |
| Buildroot (краткий рецепт make) | `make buildroot-init` | [GUIDE_BUILDROOT.md](doc/GUIDE_BUILDROOT.md) |
...
├── buildroot_zynq-mini/my_external_tree/ # BR2 external для ZYNQ MINI Rev B
...
Полный пошаговый путь Linux (только после [Vivado-мануала](doc/GUIDE_VIVADO_VITIS_FROM_SCRATCH.md) §18): **[GUIDE_LINUX_ZYNQ_MINI_OLED.md](doc/GUIDE_LINUX_ZYNQ_MINI_OLED.md)** — FSBL, Buildroot, `bootgen`, консоль на OLED создаются по шагам, без `make vitis-build` / `deploy.sh`.

- **Device Tree:** `linux/dts/zynq-mini-revb.dts` — узел custom I²C master в PL, SSD1306 @ `0x3c`, `fbcon=font:MINI4x6`
- **Драйвер (in-tree пакет):** `linux/drivers/i2c-master-axi/` — `i2c_adapter`, IRQ/poll, runtime-частота шины через sysfs `bus_hz`
- **Out-of-tree (альтернатива):** `driver/i2c-zynq-master.c` — [DRIVER.md](doc/DRIVER.md)
- **Userspace:** `linux/userspace/oled-clock/` — часы и переключение clock ↔ **системная консоль** на `/dev/fb0` (`oled-console.sh`)
...
| Документ | Описание |
|----------|----------|
| **[GUIDE_VIVADO_VITIS_FROM_SCRATCH.md](doc/GUIDE_VIVADO_VITIS_FROM_SCRATCH.md)** | Главный пошаговый гайд: AXI4-Lite, RTL, BD, симуляция §5.7, Vitis bare-metal |
| **[GUIDE_LINUX_ZYNQ_MINI_OLED.md](doc/GUIDE_LINUX_ZYNQ_MINI_OLED.md)** | Linux после Vivado §18: FSBL, Buildroot, fbcon на OLED (без готовых артефактов) |
