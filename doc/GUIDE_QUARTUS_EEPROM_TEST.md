...
Модуль `ax_debounce` работает по принципу «ждать стабильности». Два D-триггера формируют 2-стадийный синхронизатор и одновременно выступают как edge-детектор: если их значения расходятся — вход «дрогнул», и счётчик сбрасывается в 0. Только когда счётчик дошёл до `TIMER_MAX = CLK_FREQ_HZ/1000 × DEBOUNCE_MS` (для 50 МГц и 20 мс — это 1 000 000 тактов), модуль фиксирует устаканенное значение в `btn_o`. Разрядность счётчика вычисляется автоматически через `$clog2(TIMER_MAX+1)` — поменяйте параметры и ширина пересчитается без правок в коде.
...
|-------|-------------|----------|
| `btn_o` | Уровень | Стабильное текущее состояние кнопки (active-low) |
| `btn_released_o` | 1 такт | Импульс при отпускании кнопки (0→1) |
| `btn_pressed_o`  | 1 такт | Импульс при нажатии кнопки   (1→0) |

На плате AX301 кнопки active-low: нажатие = LOW, отпускание = HIGH. Мы используем `btn_pressed_o` — однотактный импульс в момент **нажатия** кнопки. Именно этот сигнал подаётся в контроллер тестов как `start`.
...
btn_pressed_o:    ─────────┐_______________________________
                            │← 1 такт
...
ax_debounce u_debounce (
    .clk_i           (clk),
    .rstn_i          (rst_n),       // active-low, как и весь проект
    .btn_i           (key1),
    .btn_o           (),            // ← устаканенный уровень не нужен
    .btn_pressed_o   (btn_negedge), // 1-тактный pulse в момент нажатия (1→0)
    .btn_released_o  ()             // ← отпускание не нужно
);
```

Модуль `ax_debounce` использует тот же стиль, что и весь проект: суффиксы `_i`/`_o` на портах и **active-LOW** асинхронный сброс `rstn_i`. Поэтому никаких инверсий — системный `rst_n` подаётся напрямую.

Два порта `.btn_o()` и `.btn_released_o()` подключены к пустым скобкам — эти выходы не используются. В Verilog пустые скобки означают «подключить к ничему» (dangling wire). Синтезатор Quartus оптимизирует неиспользуемую логику и просто не создаёт для неё схему.

Имя экземпляра `u_debounce` — конвенция «`u_`-prefix» распространена в RTL-дизайне. Она помогает отличать экземпляры модулей от обычных сигналов при чтении иерархических путей: `u_debounce.btn_o` очевидно относится к подмодулю, а не к регистру верхнего уровня.
...
└── src/
    ...
    (ax_debounce.v живёт в общем rtl/, подключается из QSF)
...
set_global_assignment -name VERILOG_FILE ../rtl/ax_debounce.v
